...
const CONFIG_COLUMNS: &str =
    "guild_id, announcements_channel_id, digest_enabled, last_digest_sent_at";
...
pub async fn upsert_config(
    ...
    let config = sqlx::query_as::<_, BotChannelConfig>(&format!(
        r#"
        INSERT INTO discord_bot_config (guild_id, announcements_channel_id, digest_enabled, updated_at)
        VALUES ($1, $2, COALESCE($3, false), $4)
        ON CONFLICT (guild_id) DO UPDATE SET
            announcements_channel_id = EXCLUDED.announcements_channel_id,
            digest_enabled = COALESCE($3, discord_bot_config.digest_enabled),
...
mod tests {
    ...
    async fn upsert_creates_then_updates_the_same_row(db: PgPool) {
        let first = upsert_config(&db, "g1", None, None).await.unwrap();
        assert!(first.announcements_channel_id.is_none());
        assert!(!first.digest_enabled);

        let second = upsert_config(&db, "g1", Some("222".to_string()), None)
            .await
            .unwrap();
    ...
    async fn resolve_announcement_channel_falls_back_when_db_value_is_unset(db: PgPool) {
        // A row exists (the digest toggle put it there) but no channel is set.
        upsert_config(&db, "g1", None, Some(true)).await.unwrap();
